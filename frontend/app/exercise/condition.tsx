...
import { exerciseEquipmentLabels, exerciseLocationLabels, initialExerciseCondition, type ExerciseEquipment, type ExerciseLocation } from '@/src/features/exercise/exerciseData';
...
export default function ExerciseConditionScreen() {
  ...
  const layout = {
    cardHeight: verticalValue(100, 96), cardPadding: verticalValue(12.5, 11), ctaTop,
    choiceHeight: verticalValue(40, 38),
    discomfortHeight: verticalValue(124, 120), discomfortPadding: verticalValue(10, 9), equipmentButtonHeight: verticalValue(65, 62),
    equipmentHeight: verticalValue(138, 130), equipmentPadding: verticalValue(14, 12), formGap: verticalValue(10, 10),
    formTop: verticalValue(200, 185), inputHeight: verticalValue(34, 32), introDescriptionMargin: verticalValue(9, 6), introTop: verticalValue(100, 85),
  };
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => discomfortInputRef.current?.measureInWindow((_x, y, _width, height) => {
      const nextOffset = Math.max(0, y + height + keyboardContentOffsetRef.current - (keyboardTop - keyboardSafeGap));
  ...
  return (
    <ExerciseScreenFrame contentHeight={contentHeight}>
      <View style={{ transform: [{ translateY: -keyboardContentOffset }] }}>
        <BackButton onPress={() => router.replace('/exercise')} style={[styles.back, { top: titleTop - 15 }]} />
        ...
        <View style={[styles.form, { gap: layout.formGap, top: layout.formTop }]}>
          <FormCard Icon={ClockFillIcon} style={{ height: layout.cardHeight, paddingVertical: layout.cardPadding }} title="운동 가능 시간">
            <View style={styles.row}>{times.map((minutes) => <Choice height={layout.choiceHeight} key={minutes} selected={condition.availableMinutes === minutes} label={minutes === 120 ? '120분 이상' : `${minutes}분`} onPress={() => setCondition({ ...condition, availableMinutes: minutes })} />)}</View>
          </FormCard>
          <FormCard Icon={MapPinIcon} style={{ height: layout.cardHeight, paddingVertical: layout.cardPadding }} title="운동 장소">
            <View style={styles.row}>{locations.map(({ Icon, id }) => <Choice height={layout.choiceHeight} key={id} Icon={Icon} selected={condition.location === id} label={exerciseLocationLabels[id]} onPress={() => setCondition({ ...condition, location: id })} />)}</View>
          </FormCard>
          <FormCard Icon={BarbellFillIcon} style={{ height: layout.equipmentHeight, paddingVertical: layout.equipmentPadding }} title={<>사용 가능한 장비 <Text style={styles.titleAside}>(복수 선택)</Text></>}>
            <View style={styles.equipmentRow}>{equipment.map((id) => {
              ...
              return <Pressable key={id} onPress={() => toggleEquipment(id)} style={[styles.equipmentChoice, { height: layout.equipmentButtonHeight }, selected && styles.choiceSelected]}>
                <EquipmentIcon color={selected ? colors.primary : colors.textSecondary} height={25} width={25} />
          ...
          <FormCard Icon={FirstAidIcon} style={{ height: layout.cardHeight, paddingVertical: layout.cardPadding }} title="현재 컨디션">
            <TextInput ref={conditionInputRef} onBlur={handleInputBlur} onChangeText={(value) => setCondition({ ...condition, condition: value })} onFocus={() => handleInputFocus('condition')} placeholder="예) 좋음, 보통, 나쁨, 매우 피곤 등" placeholderTextColor={colors.textDisabled} style={[styles.input, { height: layout.inputHeight }]} value={condition.condition} />
          </FormCard>
          <FormCard description={'운동 시 불편하거나 주의가 필요한 부위를 알려주세요.\n없을 시 생략 가능합니다.'} Icon={HandHeartFillIcon} style={{ height: layout.discomfortHeight, paddingVertical: layout.discomfortPadding }} title={<>운동 시 불편한 부위 <Text style={styles.titleAside}>(선택)</Text></>}>
            <TextInput ref={discomfortInputRef} onBlur={handleInputBlur} onChangeText={(value) => setCondition({ ...condition, discomfortArea: value })} onFocus={() => handleInputFocus('discomfort')} placeholder="예) 오른쪽 무릎, 허리, 어깨 등" placeholderTextColor={colors.textDisabled} style={[styles.input, { height: layout.inputHeight }]} value={condition.discomfortArea} />
          </FormCard>
        </View>
...
function Choice({ height, Icon, label, onPress, selected }: { height: number; Icon?: ComponentType<any>; label: string; onPress: () => void; selected: boolean }) {
  return <Pressable onPress={onPress} style={[styles.choice, { height }, selected && styles.choiceSelected]}>{Icon ? <Icon color={selected ? colors.primary : colors.textSecondary} height={16} width={16} /> : null}<Text style={[styles.choiceText, selected && styles.choiceTextSelected]}>{label}</Text>{selected ? <SelectionCheck /> : null}</Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  choice: { alignItems: 'center', backgroundColor: colors.surface, borderColor: colors.border, borderRadius: 7, borderWidth: 1, flex: 1, flexDirection: 'row', gap: 3, justifyContent: 'center', paddingHorizontal: 3, position: 'relative' },
