...
1. 백엔드 배포 완료: 2026-09-17, `https://auto-fit-api-dev.onrender.com`, 코드 `93d83bc`. 프론트에서 새 API를 사용할 수 있다.
...
배포 후 `/health` 200, 토큰 없는 `/api/home` 401, 배포 OpenAPI와 로컬 홈 응답 규격의 일치를 확인했다.
Render 배포 ID는 `dep-daljtfrl550s73bhi040`이며 `Deploy succeeded / Live`를 확인했다.
인증된 사용자 데이터의 실제 200 응답과 앱의 5초 지연 개선 폭은 아직 측정하지 않았다. 프론트 연동 후 홈 진입 → 요청 시작 →
