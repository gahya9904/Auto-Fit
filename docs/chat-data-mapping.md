...
보관기간 만료·자동 삭제 작업은 아직 활성화하지 않았다.
...
팀 확정 사항은 '1개월 보관'이다. 앞선 안내의 '메시지별 30일'은 구현 제안이며,
달력상 1개월인지 고정 30일인지와 메시지별/채팅방별 기산점은 삭제 작업 전에 확정한다.
그 전에는 자동 삭제를 활성화하지 않는다.
메시지·답변·근거 사본과 빈 채팅방의 정리 기준을 함께 정하고, 건강 원본 데이터는 보존한다.
