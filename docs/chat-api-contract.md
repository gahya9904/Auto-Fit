...
구현 상태(2026-09-08): 저장 API 5개와 DB 함수 구현 및 실제 Supabase 통합 테스트 24개 완료.
`20260908012312_chat_message_storage.sql`을 Auto-Fit 프로젝트에 적용했다.
FastAPI는 로컬 ASGI 앱으로 실행해 원격 Auth/DB를 검증했다. 배포 서버·프론트 화면 검증은 별도다.
AI 연동, 요청 제한(429), 1개월 만료 처리는 아직 구현하지 않았다.
...
- 대화 보관기간은 1개월이다. 정확한 만료 기산점과 삭제 구현은 [데이터 매핑 문서](chat-data-mapping.md)의 보관기간 항목을 따른다.
